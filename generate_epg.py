...
# === 2️⃣ Extract first 33 songs from table (history) ===
rows = soup.select("table tr")
songs = []

for row in rows[1:34]:  # take 33 songs only
...
# === 3️⃣ Prepare start and stop times ===
...
for i in range(len(start_times)):
    if i + 1 < len(start_times):
        ...
        stop_times.append(start_times[i] + datetime.timedelta(minutes=2))  # last song default
...
xml = [
    ...
    f'source-info-url="https://sgolden58.github.io/radio/epg.xml?channel_id=988&amp;date={now.strftime("%Y_
